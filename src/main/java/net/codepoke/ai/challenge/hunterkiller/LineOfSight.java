...
public class LineOfSight {
  ...
  private void compute(int octant, MapLocation origin, int rangeLimit, int x, Slope top, Slope bottom) {
    ...
    for(; x <= (int)rangeLimit; x++) // (x <= (int)rangeLimit) == (rangeLimit < 0 || x <= rangeLimit)
    {
      ...
      {
        // get the tile that the top vector enters from the left. since our coordinates refer to the centre of the
        ...
        if(blocksLight(x, topY, octant, origin)) // if the tile blocks light (i.e. is a wall)...
        {
          ...
          // corner, then it is blocked. the corner is bevelled if the tiles above and to the left are not walls. we can
        ...
        else // the tile doesn't block light
        {
          ...
          // the bottom-right corner of the tile above, 2) the tile above blocks light and does not have a bevelled
          // bottom-right corner, in which case the vector must be above the bottom-right corner, and 3) the tile above
          // blocks light and does have a bevelled bottom-right corner, in which case the vector must be above the
          ...
          // above the bottom centre. this works because if the tile above and to the right is a wall, then there are
          ...
          // is not a wall, then we again have two cases: 1) the tile above is a wall with a bevelled edge, in which
          // case we must check against the bottom centre, or 2) the tile above is not a wall, in which case it will
          ...
          if(x != rangeLimit) // but don't bother adjusting them if this is the last column anyway
          {
            if(isOpaque) {
              ...
              {                  // so adjust the bottom vector upward and continue processing it in the next column
                ...
                // otherwise, move it up to the top left. the corner is bevelled if the tiles above and to the left are
              ...
              if(wasOpaque > 0) // if we found a transition from opaque to clear, adjust the top vector downwards
              {
                ...
                // otherwise, move it down to the bottom right. the corner is bevelled if the tiles below and to the right
